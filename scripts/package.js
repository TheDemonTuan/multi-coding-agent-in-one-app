...
try {
  // Step 1: Build both React app and Electron in one vite build
  console.log('🔨 Building application (client + electron)...');
  execSync('bun run build', { cwd: rootDir, stdio: 'inherit' });

  // Step 2: Manual packaging with ASAR
  console.log('\n📦 Creating ASAR bundle...\n');

  // Use timestamp-based folder name to avoid file lock issues
  ...
  // Try to clean up legacy folder (but don't fail if it's locked)
  if (fs.existsSync(legacyReleaseDir)) {
    try {
      execSync(`powershell -Command "Remove-Item -Path '${legacyReleaseDir.replace(/'/g, "''")}' -Recurse -Force"`, {
        stdio: 'pipe',
        maxBuffer: 10 * 1024 * 1024
      });
    } catch (e) {
      // Ignore - legacy folder will be cleaned manually later
  ...
  try {
    execSync(`powershell -Command "Remove-Item -Path '${appTempDir.replace(/'/g, "''")}' -Recurse -Force"`, {
      stdio: 'pipe',
      maxBuffer: 10 * 1024 * 1024
    });
  } catch (e) {
  ...
  // Create/update symbolic link 'win-unpacked' -> latest build
  const linkPath = path.join(rootDir, 'release', 'win-unpacked');
  try {
    if (fs.existsSync(linkPath)) {
      fs.rmSync(linkPath, { recursive: true, force: true });
    }
    // Create junction point (works better on Windows than symlinks)
  ...
  try {
    ...
    for (let i = 2; i < releaseFiles.length; i++) {
      ...
      try {
        execSync(`powershell -Command "Remove-Item -Path '${oldFolder.replace(/'/g, "''")}' -Recurse -Force"`, {
          stdio: 'pipe',
          maxBuffer: 10 * 1024 * 1024
        });
